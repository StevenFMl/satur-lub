...
-- SATURLUB - MIGRACION CORREGIDA PARA SCHEMA snake_case
BEGIN;
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema='public' AND table_name='products' AND column_name='tax_rate'
  ) THEN
    EXECUTE 'UPDATE public.products SET has_tax = (COALESCE(tax_rate,0) > 0) WHERE has_tax IS DISTINCT FROM (COALESCE(tax_rate,0) > 0)';
  ELSIF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='products' AND column_name='taxrate'
  ) THEN
    EXECUTE 'UPDATE public.products SET has_tax = (COALESCE(taxrate,0) > 0) WHERE has_tax IS DISTINCT FROM (COALESCE(taxrate,0) > 0)';
  ELSE
    RAISE NOTICE 'products no tiene tax_rate ni taxrate';
  END IF;
END $$;
...
ALTER TABLE public.inventory_movements
  ADD COLUMN IF NOT EXISTS direction numeric(2,0) NOT NULL DEFAULT 1;

ALTER TABLE public.inventory_movements DROP CONSTRAINT IF EXISTS inventory_movements_direction_check;
ALTER TABLE public.inventory_movements
  ADD CONSTRAINT inventory_movements_direction_check CHECK (direction IN (1,-1));

ALTER TABLE public.inventory_movements DROP CONSTRAINT IF EXISTS inventory_movements_movement_type_check;
ALTER TABLE public.inventory_movements
  ADD CONSTRAINT inventory_movements_movement_type_check CHECK (movement_type IN ('in','out','adjustment','sale','purchase','transfer','purchase_cancel'));

DROP FUNCTION IF EXISTS public.record_stock_adjustment(uuid, uuid, uuid, numeric, numeric, text, uuid);
CREATE OR REPLACE FUNCTION public.record_stock_adjustment(
  p_tenant_id uuid,
  p_warehouse_id uuid,
  p_product_id uuid,
...
BEGIN
  ...
  IF p_quantity IS NULL OR p_quantity <= 0 THEN RAISE EXCEPTION 'p_quantity inválido'; END IF;
  IF p_unit_cost IS NULL OR p_unit_cost < 0 THEN RAISE EXCEPTION 'p_unit_cost inválido'; END IF;

  INSERT INTO public.inventory_movements (
    tenant_id, warehouse_id, product_id, movement_type,
    quantity, unit_cost, reason, performed_by_user_id, direction
  ) VALUES (
    p_tenant_id, p_warehouse_id, p_product_id, 'adjustment',
    p_quantity, p_unit_cost, p_reason, p_performed_by_user_id, 1
  );

  INSERT INTO public.inventory_balances (
    tenant_id, warehouse_id, product_id, quantity_on_hand
  ) VALUES (
    p_tenant_id, p_warehouse_id, p_product_id, p_quantity
  )
  ON CONFLICT (tenant_id, warehouse_id, product_id)
  DO UPDATE SET
    quantity_on_hand = public.inventory_balances.quantity_on_hand + EXCLUDED.quantity_on_hand,
...
DECLARE
  ...
  v_calc_tax_amount numeric(14,2) := 0;
  v_calc_grand_total numeric(14,2) := 0;
  v_product_has_tax boolean;
  v_pay_status text;
BEGIN
  IF v_user_id IS NULL THEN RAISE EXCEPTION 'Usuario no autenticado'; END IF;
  IF p_tenant_id IS NULL THEN RAISE EXCEPTION 'p_tenant_id es requerido'; END IF;
  IF jsonb_typeof(p_items) <> 'array' OR jsonb_array_length(p_items) = 0 THEN RAISE EXCEPTION 'Debe incluir al menos un ítem'; END IF;
  IF p_payment_method NOT IN ('cash','transfer','credit') THEN RAISE EXCEPTION 'Método de pago inválido'; END IF;
  IF p_tax_rate IS NULL OR p_tax_rate < 0 OR p_tax_rate > 100 THEN RAISE EXCEPTION 'Tasa de IVA inválida'; END IF;
  IF p_subtotal IS NULL OR p_tax_amount IS NULL OR p_grand_total IS NULL OR p_other_charges IS NULL THEN RAISE EXCEPTION 'Totales inválidos'; END IF;

  IF NOT EXISTS (
    SELECT 1 FROM public.business_partners
    WHERE id = p_supplier_id AND tenant_id = p_tenant_id AND partner_type = 'supplier' AND is_active = true
  ) THEN RAISE EXCEPTION 'Proveedor inválido'; END IF;

  IF p_warehouse_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.warehouses WHERE id = p_warehouse_id AND tenant_id = p_tenant_id
  ) THEN RAISE EXCEPTION 'Bodega inválida'; END IF;

  v_pay_status := CASE WHEN p_payment_method IN ('cash','transfer') THEN COALESCE(p_payment_status,'paid') ELSE COALESCE(p_payment_status,'pending') END;
  ...
  LOOP
    ...
    IF v_product_id IS NULL THEN RAISE EXCEPTION 'Ítem sin product_id'; END IF;
    IF v_qty IS NULL OR v_qty <= 0 THEN RAISE EXCEPTION 'Cantidad inválida'; END IF;
    IF v_unit_cost IS NULL OR v_unit_cost < 0 THEN RAISE EXCEPTION 'Costo unitario inválido'; END IF;

    SELECT p.has_tax INTO v_product_has_tax
    FROM public.products p
    WHERE p.id = v_product_id AND p.tenant_id = p_tenant_id;

    IF NOT FOUND THEN RAISE EXCEPTION 'Producto inválido %', v_product_id; END IF;

    v_line_total := round(v_qty * v_unit_cost, 2);
    v_calc_subtotal := v_calc_subtotal + v_line_total;
    IF v_is_taxable AND COALESCE(v_product_has_tax, true) AND p_tax_rate > 0 THEN
      v_calc_tax_amount := v_calc_tax_amount + round(v_line_total * (p_tax_rate / 100.0), 2);
    END IF;
  END LOOP;

  v_calc_subtotal := round(v_calc_subtotal, 2);
  v_calc_tax_amount := round(v_calc_tax_amount, 2);
  v_calc_grand_total := round(v_calc_subtotal + v_calc_tax_amount + p_other_charges, 2);

  IF abs(v_calc_subtotal - p_subtotal) > 0.02 THEN RAISE EXCEPTION 'Subtotal inconsistente'; END IF;
  IF abs(v_calc_tax_amount - p_tax_amount) > 0.02 THEN RAISE EXCEPTION 'IVA inconsistente'; END IF;
  IF abs(v_calc_grand_total - p_grand_total) > 0.02 THEN RAISE EXCEPTION 'Total inconsistente'; END IF;

  INSERT INTO public.purchase_orders (
    tenant_id, supplier_id, warehouse_id, subtotal, tax_total, total,
    tax_rate, is_tax_inclusive, tax_amount, grand_total, other_charges,
    status, notes, created_by, payment_method, payment_status, payment_due_date
  ) VALUES (
    p_tenant_id, p_supplier_id, p_warehouse_id, p_subtotal, p_tax_amount, p_grand_total,
    p_tax_rate, p_is_tax_inclusive, p_tax_amount, p_grand_total, p_other_charges,
    'received', p_notes, v_user_id, p_payment_method, v_pay_status,
    CASE WHEN p_payment_method = 'credit' THEN p_payment_due_date ELSE NULL END
  ) RETURNING id INTO v_po_id;
  ...
  LOOP
    ...
    INSERT INTO public.purchase_order_items (
      tenant_id, purchase_order_id, product_id, quantity, unit_cost, line_total, is_taxable
    ) VALUES (
      p_tenant_id, v_po_id, v_product_id, v_qty, v_unit_cost, v_line_total, v_is_taxable
    );

    INSERT INTO public.inventory_movements (
      tenant_id, warehouse_id, product_id, movement_type,
      quantity, unit_cost, reason, reference_type, reference_id,
      performed_by_user_id, direction
    ) VALUES (
      p_tenant_id, p_warehouse_id, v_product_id, 'purchase',
      v_qty, v_unit_cost, 'Recepción de compra', 'purchase_order', v_po_id,
      v_user_id, 1
    );
...
BEGIN
  ...
  FROM public.purchase_orders
  WHERE id = p_purchase_order_id AND tenant_id = p_tenant_id;

  IF NOT FOUND THEN RAISE EXCEPTION 'Orden de compra no encontrada'; END IF;
  IF v_status <> 'received' THEN RAISE EXCEPTION 'Solo se pueden anular órdenes en estado received'; END IF;
  ...
  FOR v_item IN
    SELECT product_id, quantity, unit_cost
    FROM public.purchase_order_items
    WHERE purchase_order_id = p_purchase_order_id AND tenant_id = p_tenant_id
  LOOP
    INSERT INTO public.inventory_movements (
      tenant_id, warehouse_id, product_id, movement_type,
      quantity, unit_cost, reason, reference_type, reference_id,
      ...
      p_tenant_id, v_warehouse_id, v_item.product_id, 'purchase_cancel',
      v_item.quantity, v_item.unit_cost, 'Anulación de compra', 'purchase_order', p_purchase_order_id,
      v_user_id, -1
    );
    ...
    IF v_warehouse_id IS NOT NULL THEN
      ...
      SET quantity_on_hand = quantity_on_hand - v_item.quantity,
          updated_at = now()
      WHERE tenant_id = p_tenant_id AND warehouse_id = v_warehouse_id AND product_id = v_item.product_id;
    END IF;
  END LOOP;
END;
$$;
