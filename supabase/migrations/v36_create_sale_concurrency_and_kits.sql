-- ============================================================
-- v36_create_sale_concurrency_and_kits.sql
--
-- Robust database-driven components resolving for kits/bundles
-- and concurrent unique_violation safety on idempotency keys.
-- ============================================================

-- ── 1. Re-define create_sale RPC with full concurrency and DB-driven kits ──

DROP FUNCTION IF EXISTS public.create_sale(uuid, uuid, uuid, jsonb, jsonb, text, text, date, uuid, boolean, numeric, text, text, date, text, boolean, numeric, uuid);
...
BEGIN
  ...
  IF p_warehouse_id IS NOT NULL THEN
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
      ...
      -- Fetch product info from DB to resolve product kind
      SELECT p.product_kind, p.track_inventory, p.name
        INTO v_parent_kind, v_parent_track_inv, v_product_name
        FROM products p
      ...
      -- If it's a kit/bundle, query components directly from bundle_items (Database Authority)
      IF v_parent_kind IN ('kit', 'bundle') THEN
        ...
        -- Prevent selling kits/bundles without configured components
        IF NOT EXISTS (
          SELECT 1 FROM public.bundle_items
           WHERE bundle_product_id = v_product_id AND tenant_id = p_tenant_id
        ) THEN
          RAISE EXCEPTION 'El kit/bundle "%" no tiene componentes configurados en el sistema.', v_product_name;
        END IF;
        ...
        FOR v_comp_rec IN
          SELECT bi.component_product_id,
            ...
            JOIN public.products p ON p.id = bi.component_product_id AND p.tenant_id = p_tenant_id AND p.is_active = true
           WHERE bi.bundle_product_id = v_product_id AND bi.tenant_id = p_tenant_id
        LOOP
          v_comp_product_id := v_comp_rec.component_product_id;
          v_comp_quantity   := v_comp_rec.comp_quantity * v_quantity;
          v_comp_base_qty   := COALESCE(v_comp_rec.comp_base_qty, 1);
          v_comp_inv_qty    := v_comp_quantity * v_comp_base_qty;
          v_comp_track_inv  := v_comp_rec.comp_track_inv;
          v_comp_name       := v_comp_rec.comp_name;
  ...
  -- ── Insert sale header with concurrency safety ────────────────────────
  BEGIN
    INSERT INTO sales (
