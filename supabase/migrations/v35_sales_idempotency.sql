-- ============================================================
-- v35_sales_idempotency.sql
--
-- Transactional idempotency for create_sale: prevents duplicate
-- sales when a client retries after a network timeout where the
-- server already committed the transaction.
--
-- MECHANISM:
--   1. A new nullable column `idempotency_key uuid` is added to
--      `sales`.  A partial unique index enforces uniqueness
--      per tenant only for non-NULL values, so legacy rows are
--      unaffected.
--
--   2. create_sale accepts an optional `p_idempotency_key uuid`.
--      If supplied and a matching sale already exists for that
--      tenant, the function returns the existing sale ID
--      immediately — no writes, no inventory changes, no cash
--      movements.
--
--   3. On first call the key is stored in the INSERT INTO sales.
--      On any subsequent retry the SELECT hits the unique index
--      and returns in O(log n).
--
-- RACE CONDITION:
--   Two concurrent first-calls with the same key will collide on
--   the unique index.  The second call gets a unique_violation
--   (SQLSTATE 23505).  The client should treat this as a normal
--   retry: wait a moment and re-call — the first call will have
--   committed by then and the SELECT guard will return the ID.
--
-- BACKWARD COMPATIBILITY:
--   p_idempotency_key DEFAULT NULL → callers that do not supply
--   the key continue to work exactly as before.
-- ============================================================
...
-- Partial unique index: NULLs are excluded so legacy rows with
-- idempotency_key IS NULL do not interfere with each other.
CREATE UNIQUE INDEX IF NOT EXISTS idx_sales_tenant_idempotency
  ON public.sales (tenant_id, idempotency_key)
  WHERE idempotency_key IS NOT NULL;

-- ── 2. Replace create_sale (drop v34's 17-param signature) ───────────────

DROP FUNCTION IF EXISTS public.create_sale(uuid, uuid, uuid, jsonb, jsonb, text, text, date, uuid, boolean, numeric, text, text, date, text, boolean, numeric);
...
BEGIN
  ...
  -- If a sale with this key was already committed, return its ID immediately.
...
REVOKE ALL ON FUNCTION public.create_sale(uuid, uuid, uuid, jsonb, jsonb, text, text, date, uuid, boolean, numeric, text, text, date, text, boolean, numeric, uuid) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.create_sale(uuid, uuid, uuid, jsonb, jsonb, text, text, date, uuid, boolean, numeric, text, text, date, text, boolean, numeric, uuid) TO authenticated;
