...
-- v15_bundle_type.sql
--
-- Adds "bundle" as a valid product_kind alongside "kit":
--
--   · Drops the old CHECK constraint on products.product_kind
--     that only allowed ('item', 'service', 'kit').
--   · Adds a new constraint that also permits 'bundle'.
--   · Replaces create_sale to accept components on both kit
--     and bundle products (one-line guard change from v14).
--
-- Semantic distinction (UI only — same fulfillment logic):
--   kit    = service package  (Cambio de aceite, Alineación+balanceo)
--   bundle = commercial combo (Paquete aceite + filtro + aditivo)
--
-- Both types: track_inventory = false, components in bundle_items.
-- ============================================================


-- ── 1. Extend products.product_kind constraint ───────────────────────────
-- PostgreSQL inline CHECK constraints get auto-named
-- "<table>_<column>_check".  Drop-and-recreate is the only way to alter it.
...
ALTER TABLE public.products
  ...
  CHECK (product_kind IN ('item', 'service', 'kit', 'bundle'));


-- ── 2. Updated create_sale — guard accepts kit + bundle ───────────────────
...
DECLARE
  ...
  v_comp_name         text;
  v_kit_name          text;
  -- kit/bundle parent validation (server-side guard, independent of UI)
  v_parent_kind       text;
...
BEGIN
  ...
  IF p_warehouse_id IS NOT NULL THEN
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
      ...
      THEN
        ...
        -- Rejects any payload that passes components for a non-kit/bundle product.
        ...
        -- ── Validate each component's stock ──────────────────────────
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- ── sale_items row (always — kit/bundle or not) ──────────────────
    ...
    IF v_components IS NOT NULL
       ...
       AND jsonb_array_length(v_components) > 0
    THEN
      -- Kit/bundle: decrement each component's stock, not the parent product
      FOR v_comp IN SELECT * FROM jsonb_array_elements(v_components) LOOP
        v_comp_product_id := (v_comp->>'product_id')::uuid;
        v_comp_quantity   := (v_comp->>'quantity')::numeric * v_quantity;
        v_comp_base_qty   := COALESCE((v_comp->>'base_qty')::numeric, 1);
    ...
    ELSE
      -- Regular item: standard stock decrement
