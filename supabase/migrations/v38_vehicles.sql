...
-- v38_vehicles.sql
--
-- Fase 2: vehículos en el POS.
--
-- CAMBIOS:
--   1. Tabla `vehicles` — un vehículo pertenece a un cliente
--      (business_partners), placa única por tenant.
--   2. Columna `vehicle_id` nullable en `sales` — enlace al
--      vehículo atendido.  Nullable: la venta sigue funcionando
--      sin vehículo (walk-in, servicios, productos varios).
--   3. create_sale actualizado para aceptar `p_vehicle_id`
--      opcional y escribirlo atómicamente en el INSERT.
--
-- BACKWARD COMPATIBILITY:
--   Todos los cambios son aditivos y nullable/DEFAULT.
--   Callers existentes que no pasen p_vehicle_id siguen
...
CREATE TABLE IF NOT EXISTS public.vehicles (
  ...
  -- Placa única dentro del tenant
  CONSTRAINT uq_vehicles_plate UNIQUE (tenant_id, plate),

  -- FK compuesta (tenant_id, partner_id) al patrón del proyecto
  CONSTRAINT fk_vehicles_partner FOREIGN KEY (tenant_id, partner_id)
    REFERENCES public.business_partners (tenant_id, id) ON DELETE CASCADE
);

-- Índice para listar vehículos de un cliente rápidamente
CREATE INDEX IF NOT EXISTS idx_vehicles_tenant_partner
  ON public.vehicles (tenant_id, partner_id);

-- RLS
ALTER TABLE public.vehicles ENABLE ROW LEVEL SECURITY;

CREATE POLICY "tenant_members_select_vehicles"
...
CREATE POLICY "tenant_members_insert_vehicles"
...
CREATE POLICY "tenant_members_update_vehicles"
...
-- ── 2. vehicle_id en sales ────────────────────────────────────────────────

ALTER TABLE public.sales
  ADD COLUMN IF NOT EXISTS vehicle_id uuid
    REFERENCES public.vehicles(id) ON DELETE SET NULL;

-- Índice para historial por vehículo (Fase 3)
CREATE INDEX IF NOT EXISTS idx_sales_vehicle
  ON public.sales (tenant_id, vehicle_id)
  WHERE vehicle_id IS NOT NULL;

-- ── 3. Actualizar create_sale (drop v35 18-param signature) ──────────────
...
REVOKE ALL ON FUNCTION public.create_sale(uuid, uuid, uuid, jsonb, jsonb, text, text, date, uuid, boolean, numeric, text, text, date, text, boolean, numeric, uuid, uuid) FROM PUBLIC;
