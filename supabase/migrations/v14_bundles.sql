...
-- v14_bundles.sql
--
-- Kit / Bundle support for POS:
--
--   · bundle_items table — components per kit product
--   · Extended create_sale RPC — handles p_items entries that carry
--     a "components" array:
--       - Stock pre-validation uses components, not the kit itself
--       - sale_items gets ONE row for the kit (correct fiscal record)
--       - inventory_movements gets one row PER component (real stock)
--       - The kit product must have track_inventory = false
--
-- Backward compatible: items without "components" behave exactly
-- as before. No existing callers need to change.
--
-- Idempotency:
--   CREATE TABLE IF NOT EXISTS · ADD COLUMN IF NOT EXISTS
--   DO $$ for policies · DROP + CREATE OR REPLACE for RPC
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.bundle_items (
  ...
  base_qty              numeric(12,4) NOT NULL DEFAULT 1 CHECK (base_qty > 0),
...
CREATE INDEX IF NOT EXISTS idx_bundle_items_bundle
  ON public.bundle_items(tenant_id, bundle_product_id);
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
     WHERE tablename = 'bundle_items' AND policyname = 'bundle_items_tenant_access'
  ) THEN
    CREATE POLICY "bundle_items_tenant_access" ON public.bundle_items
      FOR ALL
      USING (
        tenant_id IN (
...
-- ══════════════════════════════════════════════════════════════
-- 2. Extended create_sale — full replacement (v9 + bundle support)
-- ══════════════════════════════════════════════════════════════
--
-- p_items item shape (new optional field):
--   {
--     product_id, quantity, discount_amount?, presentation_id?, base_qty?,
--     override_unit_price?, price_override_type?, price_override_reason?,
--     price_override_note?,
--     "components": [            ← NEW (kit/bundle items only)
--       { "product_id": "...", "quantity": 4, "base_qty": 1 },
--       ...
--     ]
--   }
--
-- Bundle semantics:
--   · Kit product itself → goes into sale_items at kit price (fiscal).
--   · Kit components     → stock decremented via inventory_movements (real).
--   · The privilege check (30% max discount) still applies to the kit
--     product's price override, if any.  Components have no price override.
-- ══════════════════════════════════════════════════════════════

DROP FUNCTION IF EXISTS public.create_sale(
  uuid,uuid,uuid,jsonb,jsonb,text,text,date,uuid,boolean,numeric,text,text,date,text
);
...
DECLARE
  ...
  v_comp_name         text;
  v_kit_name          text;
  -- kit parent validation (server-side guard, independent of UI)
  v_parent_kind       text;
...
BEGIN
  ...
  IF p_warehouse_id IS NOT NULL THEN
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
      ...
      THEN
        -- ── Server-side kit integrity guard ──────────────────────────
        -- Reject any payload that passes components for a non-kit product.
        -- This is NOT optional: it prevents a crafted JSONB payload from
        -- silently decrementing component stock on a regular product.
        ...
        IF v_parent_kind IS DISTINCT FROM 'kit' THEN
          RAISE EXCEPTION
            'El ítem % tiene componentes pero product_kind = "%" (se requiere "kit").',
            v_product_id, COALESCE(v_parent_kind, 'NULL');
        END IF;

        IF COALESCE(v_parent_track_inv, false) = true THEN
          RAISE EXCEPTION
            'El kit % tiene track_inventory = true. Los kits no deben llevar inventario propio.',
            v_product_id;
        END IF;

        -- ── Kit/bundle: validate each component ─────────────────────
      ...
      ELSE
        -- ── Regular item: validate product stock directly ────────────
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    v_kit_name := v_product_name;  -- save for component movement reason
    ...
    -- ── sale_items row (always — kit or not) ─────────────────────────
    ...
    -- ── Inventory: kit vs regular ────────────────────────────────────
    IF v_components IS NOT NULL
       AND jsonb_typeof(v_components) = 'array'
       AND jsonb_array_length(v_components) > 0
    THEN
      -- Kit: decrement each component's stock, not the kit product itself
      FOR v_comp IN SELECT * FROM jsonb_array_elements(v_components) LOOP
        v_comp_product_id := (v_comp->>'product_id')::uuid;
        v_comp_quantity   := (v_comp->>'quantity')::numeric * v_quantity;
        v_comp_base_qty   := COALESCE((v_comp->>'base_qty')::numeric, 1);
    ...
    ELSE
      -- Regular item: existing stock decrement
  ...
  ELSE
    FOR v_payment IN SELECT * FROM jsonb_array_elements(p_payments) LOOP
      ...
      END IF;
      INSERT INTO sale_payments (tenant_id, sale_id, payment_method, amount, reference)
      VALUES (p_tenant_id, v_sale_id, v_payment->>'method',
...
-- Use the full type signature to avoid ambiguity in case of future overloads.
-- SECURITY DEFINER + SET search_path = public is the standard Supabase hardening
-- pattern: the fixed search_path eliminates schema-squatting vectors (an attacker
-- creating objects in a schema that would shadow public.*).
-- Only `authenticated` gets EXECUTE; `anon` is excluded via the FROM PUBLIC revoke.
