-- ============================================================
-- v34_below_cost_atomic.sql
--
-- Makes below_cost_override and below_cost_loss_estimated part
-- of the create_sale RPC INSERT INTO sales (atomic).
--
-- ROOT CAUSE:
--   The post-RPC UPDATE in sales.ts that wrote these two fields
--   after a successful create_sale call introduced a race window:
--   if the UPDATE failed (network blip, RLS edge-case, etc.) the
--   sale existed in the DB but the audit flag was silently lost.
--
-- FIX:
--   Add two new parameters to create_sale:
--     p_below_cost_override      boolean DEFAULT false
--     p_below_cost_loss_estimated numeric DEFAULT NULL
--
--   Pass them directly into the INSERT INTO sales column list.
--   The post-RPC UPDATE in sales.ts is deleted.
--
-- BACKWARD COMPATIBILITY:
--   Both parameters have DEFAULT values, so existing callers that
--   do not supply them continue to work unchanged — the columns
--   are simply written with their current defaults (false / NULL).
-- ============================================================
...
REVOKE ALL ON FUNCTION public.create_sale(uuid, uuid, uuid, jsonb, jsonb, text, text, date, uuid, boolean, numeric, text, text, date, text, boolean, numeric) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.create_sale(uuid, uuid, uuid, jsonb, jsonb, text, text, date, uuid, boolean, numeric, text, text, date, text, boolean, numeric) TO authenticated;
