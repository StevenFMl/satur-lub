-- v21_manual_sale_items.sql
--
-- Añade soporte nativo para ítems de venta "manuales" o "fuera de stock"
-- (sin crear producto en catálogo) mediante la flexibilización de `sale_items`.
--
-- CAMBIOS DE SCHEMA:
-- 1. Añade `item_name` a `sale_items` (snapshot obligatorio del nombre).
-- 2. Permite `product_id` NULL en `sale_items` y `sale_return_items`.
--
-- CAMBIOS AL RPC `create_sale`:
-- - Modificado para soportar líneas con `product_id = null`.
-- - Respeta el `item_name` que envía el frontend.
-- - Para ítems manuales, asume el precio e impuestos que envía el cliente,
--   no realiza validación de stock y pasa el costo estimado directo a `unit_cost`.

-- ── 1. Modificar tabla sale_items ───────────────────────────────────────────

ALTER TABLE public.sale_items
  ADD COLUMN IF NOT EXISTS item_name text;

-- Backfill histórico: el nombre del producto al momento de la migración
...
-- Para registros donde el producto pudo haber sido borrado
UPDATE public.sale_items
   SET item_name = '(Producto eliminado)'
 WHERE item_name IS NULL;

DO $$
BEGIN
  ALTER TABLE public.sale_items ALTER COLUMN item_name SET NOT NULL;
END $$;
...
-- ── 2. Actualizar RPC create_sale ───────────────────────────────────────────

DROP FUNCTION IF EXISTS public.create_sale(uuid, uuid, uuid, jsonb, jsonb, text, text, date, uuid, boolean, numeric, text, text, date, text);
...
BEGIN
  ...
  IF p_warehouse_id IS NOT NULL THEN
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
      ...
      -- Si es ítem manual sin producto, saltamos validación de stock
      ...
      THEN
        -- Server-side kit/bundle integrity guard
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_product_id IS NOT NULL THEN
      -- ÍTEM DEL CATÁLOGO: Autoridad del servidor
      ...
      v_unit_cost := NULL; -- Será capturado por el trigger v17
    ELSE
      -- ÍTEM MANUAL (NO STOCK): Autoridad del frontend + Snapshot manual
      ...
      v_track_inv    := false;
      -- Si es manual, se acepta un costo explícito si el usuario lo pone (rentabilidad)
    ...
    IF v_override_price IS NOT NULL THEN
      ...
      END IF;
      -- Validación de descuento máximo solo aplica si es un producto real con precio base > 0
    ...
    -- ── sale_items row (always — manual or kit or regular) ────────────
...
REVOKE ALL ON FUNCTION public.create_sale(uuid, uuid, uuid, jsonb, jsonb, text, text, date, uuid, boolean, numeric, text, text, date, text) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.create_sale(uuid, uuid, uuid, jsonb, jsonb, text, text, date, uuid, boolean, numeric, text, text, date, text) TO authenticated;
